...
public class AutoResizeTextView extends TextView {
    ...
    public AutoResizeTextView(final Context context, final AttributeSet attrs, final int defStyle) {
        ...
        if (CMAppGlobals.DEBUG)
            Logger.i(TAG, ":: AutoResizeTextView : _minTextSize : " + _minTextSize + " : _maxTextSize : " + _maxTextSize);
        ...
        _sizeTester = new SizeTester() {
            ...
            public int onTestSize(final int suggestedSize, final RectF availableSpace) {
                if (CMAppGlobals.DEBUG)
                    Logger.i(TAG, ":: AutoResizeTextView.onTestSize : suggestedSize : " + suggestedSize);
                if (CMAppGlobals.DEBUG)
                    Logger.i(TAG, ":: AutoResizeTextView.onTestSize : _minTextSize : " + _minTextSize + " : _maxTextSize : " + _maxTextSize);
                ...
                if (singleLine) {
                    ...
                    if (CMAppGlobals.DEBUG)
                        Logger.i(TAG, ":: AutoResizeTextView.onTestSize : getMaxLines : " + getMaxLines());
                    if (CMAppGlobals.DEBUG)
                        Logger.i(TAG, ":: AutoResizeTextView.onTestSize : NO_LINE_LIMIT : " + NO_LINE_LIMIT);
                    if (CMAppGlobals.DEBUG)
                        Logger.i(TAG, ":: AutoResizeTextView.onTestSize : layout.getLineCount : " + layout.getLineCount());
                ...
                if (CMAppGlobals.DEBUG)
                    Logger.i(TAG, ":: AutoResizeTextView.onTestSize : rect : availableSpace : " + availableSpace);
                if (CMAppGlobals.DEBUG)
                    Logger.i(TAG, ":: AutoResizeTextView.onTestSize : rect : textRect : " + textRect);
//                if (textRect.bottom > availableSpace.bottom)
//                    return -1;
//                if (textRect.bottom == availableSpace.bottom)
//                    return 0;
    ...
    private void adjustTextSize() {
        ...
        post(new Runnable() {
            ...
            public void run() {
                if (CMAppGlobals.DEBUG)
                    Logger.i(TAG, ":: AutoResizeTextView.adjustTextSize : _initialized : " + _initialized);
                if (!_initialized)
                    return;
                ...
                Logger.i(TAG, ":: AutoResizeTextView.adjustTextSize : getHeight : " + getHeight());
                Logger.i(TAG, ":: AutoResizeTextView.adjustTextSize : getMeasuredHeight : " + getMeasuredHeight());
                Logger.i(TAG, ":: AutoResizeTextView.adjustTextSize : getCompoundPaddingBottom : " + getCompoundPaddingBottom());
                Logger.i(TAG, ":: AutoResizeTextView.adjustTextSize : getCompoundPaddingTop : " + getCompoundPaddingTop());
                Logger.i(TAG, ":: AutoResizeTextView.adjustTextSize : startSize : " + startSize);
                Logger.i(TAG, ":: AutoResizeTextView.adjustTextSize : heightLimit : " + heightLimit);
                Logger.i(TAG, ":: AutoResizeTextView.adjustTextSize : _widthLimit : " + _widthLimit);
                if (_widthLimit <= 0)
                    return;
                _paint = new TextPaint(getPaint());
    ...
    private void superSetTextSize(int startSize) {
        if (CMAppGlobals.DEBUG)
            Logger.i(TAG, ":: AutoResizeTextView.superSetTextSize : startSize : " + startSize);

        int textSize = binarySearch(startSize, (int) _maxTextSize, _sizeTester, _availableSpaceRect);
        if (CMAppGlobals.DEBUG)
            Logger.i(TAG, ":: AutoResizeTextView.superSetTextSize : textSize : " + textSize);
    ...
    private int binarySearch(final int start, final int end, final SizeTester sizeTester, final RectF availableSpace) {
        if (CMAppGlobals.DEBUG)
            Logger.i(TAG, ":: AutoResizeTextView.binarySearch : start : " + start + " : end : " + end);

        int lastBest = start, lo = start, hi = end - 1, mid;
        while (lo <= hi) {
            if (CMAppGlobals.DEBUG) Logger.i(TAG, ":: AutoResizeTextView.binarySearch : while");
            mid = lo + hi >>> 1;
            if (CMAppGlobals.DEBUG)
                Logger.i(TAG, ":: AutoResizeTextView.binarySearch : mid ։ " + mid);
            final int midValCmp = sizeTester.onTestSize(mid, availableSpace);
            if (CMAppGlobals.DEBUG)
                Logger.i(TAG, ":: AutoResizeTextView.binarySearch : midValCmp : " + midValCmp);
            if (midValCmp < 0) {
                lastBest = lo;
                lo = mid + 1;
            } else if (midValCmp > 0) {
        ...
        if (CMAppGlobals.DEBUG)
            Logger.i(TAG, ":: AutoResizeTextView.binarySearch : lastBest : " + lastBest);
