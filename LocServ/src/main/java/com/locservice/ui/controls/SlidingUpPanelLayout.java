...
@SuppressLint({ "NewApi", "ClickableViewAccessibility" }) public class SlidingUpPanelLayout extends ViewGroup {
    ...
    private boolean isCanScrollToDown;
    ...
    public ViewGroup getmScrollableView() {
	...
	public SlidingUpPanelLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mScreenPoint = CMApplication.getRealScreenSize(getContext());
    }
...
//        if(CMAppGlobals.DEBUG)Logger.i(TAG, ":: SlidingUpPanelLayout.onInterceptTouchEvent : mCanSlide : " + mCanSlide + " : mIsSlidingEnabled : " + mIsSlidingEnabled);

        final int action = MotionEventCompat.getActionMasked(ev);


//        if(CMAppGlobals.DEBUG)Logger.i(TAG, ":: SlidingUpPanelLayout.onInterceptTouchEvent : mIsUnableToDrag : " + mIsUnableToDrag + " : action : " + action);
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                // check is mScrollableView can scroll to down
                boolean isCanScrollDown = mSlideOffset == 0 && (y > mInitialMotionY && isCanScrollToDown);

                if (CMAppGlobals.DEBUG) Logger.i(TAG, ":: SlidingUpPanelLayout.onInterceptTouchEvent : mScreenPoint : " + mScreenPoint);
                if (CMAppGlobals.DEBUG) Logger.i(TAG, ":: SlidingUpPanelLayout.onInterceptTouchEvent : mPanelHeight : " + mPanelHeight);

                // for not scrolling when slide panel is collapsed
                if (isCollapsed() && mScreenPoint != null && mPanelHeight != -1) {
                    float slideMinY = mScreenPoint.y - mPanelHeight - 100;
                    if (CMAppGlobals.DEBUG) Logger.i(TAG, ":: SlidingUpPanelLayout.onInterceptTouchEvent : mScreenPoint.y : " + mScreenPoint.y);
                    if (CMAppGlobals.DEBUG) Logger.i(TAG, ":: SlidingUpPanelLayout.onInterceptTouchEvent : y : " + y);
                    if (CMAppGlobals.DEBUG) Logger.i(TAG, ":: SlidingUpPanelLayout.onInterceptTouchEvent : slideMinY : " + slideMinY);
                    if (CMAppGlobals.DEBUG) Logger.i(TAG, ":: SlidingUpPanelLayout.onInterceptTouchEvent : adx : " + (adx));
                    if (CMAppGlobals.DEBUG) Logger.i(TAG, ":: SlidingUpPanelLayout.onInterceptTouchEvent : ady : " + (ady));
                    if (y > slideMinY && ady > 5)
                        return true;
                }

                if (isCanScrollDown || isCanScrollUp || (ady > dragSlop && adx > ady) || !isDragViewUnder((int) x, (int) y)) {
                    mDragHelper.cancel();
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (CMAppGlobals.DEBUG) Logger.d(TAG, ":: SlidingUpPanelLayout.onTouchEvent : mCanSlide : " + mCanSlide + " : mIsSlidingEnabled : " + mIsSlidingEnabled);
     ...
     * @return true if the pane was slidable and is now expanded/in the process of expanding
     */
    ...
    /**
     * Check if the layout is completely collapsed.
     *
     * @return true if sliding panels are completely collapsed
     */
...
//        mSlideState = SlideState.EXPANDED;
            ...
            public void run() {
                ...
                observer.addOnScrollChangedListener(new ViewTreeObserver.OnScrollChangedListener(){
                    ...
                    public void onScrollChanged() {
                        ...
                        if (CMAppGlobals.DEBUG) Logger.i(TAG, ":: SlidingUpPanelLayout.setScrollableView : scrollX : " + scrollX + " : scrollY : " + scrollY);
                        if (scrollY == 0) {
                            isCanScrollToDown = false;
                        } else {
                            isCanScrollToDown = true;
                        }

                    }});
            }});

        if (CMAppGlobals.DEBUG) Logger.i(TAG, ":: SlidingUpPanelLayout.setScrollableView : mScrollableView : " + mScrollableView);
    }
	...
	public synchronized void setPanelState(SlideState state) {
    	if(CMAppGlobals.DEBUG)Logger.i(TAG, ":: SlidingUpPanelLayout.setPanelState : state : " + state);
    ...
    @SuppressWarnings("incomplete-switch")
	public void CollapsePanelFirstTime(SlideState state) {
    	if (state == null || state == SlideState.DRAGGING) {
            throw new IllegalArgumentException("Panel state cannot be null or DRAGGING.");
        }
        if (!isEnabled()
        ...
        if (mFirstLayout) {
            ...
            switch (state) {
                ...
                case EXPANDED:
                    ...
                    break;
                case HIDDEN:
                    int newTop = computePanelTopPosition(1.0f) + (mIsSlidingUp ? +mPanelHeight : -mPanelHeight);
                    smoothSlideTo(computeSlideOffset(newTop), 0);
                    break;
            }
        }
    	
    } // end method CollapsePanelFirstTime
